...
# Enable local DWS on BrightSign device via rDWS API
# Usage: ./rdws-local-dws-enable.sh <device-serial>
...
echo "Enabling local DWS on device: $DEVICE_SERIAL"
...
echo ""
echo "Enable local DWS command sent. The device may reboot to apply this change."