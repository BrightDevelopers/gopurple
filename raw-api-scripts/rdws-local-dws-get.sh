...
# Get local DWS status on BrightSign device via rDWS API
# Usage: ./rdws-local-dws-get.sh <device-serial>
...
echo "Getting local DWS status for device: $DEVICE_SERIAL"

curl -X GET "https://ws.bsn.cloud/rest/v1/control/local-dws/?destinationType=player&destinationName=${DEVICE_SERIAL}" \
